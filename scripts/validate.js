...
class FormValidator {
  ...
  _setEventListeners() {
    const inputList = Array.from(this._formElement.querySelectorAll(this._inputSelector));//все инпуты формы в массив
    const buttonElement = this._formElement.querySelector(this._submitButtonSelector);//кнопку формы в переменную
    ...
    this._inputList.forEach((inputListElement) => { 
      inputListElement.addEventListener("input", () => { //проходим по всем инпутам формы и добавляем обработчик событий, который
  ...
  _isValid(inputListElement) {
    if (!inputListElement.validity.valid) { //если valid false(невалидный), то
    this._showInputError(inputListElement, inputListElement.validationMessage); //запускает метод, показывающий невалидность визуально
  } else {
  ...
  if (this._hasInvalidInput()) {// проверяет на true валидность поля
  ...
  _showInputError(inputListElement) {
    const inputErrorContainer = this._formElement.querySelector(`.${inputListElement.id}-error`); // Находим в форме элемент спан, куда будет выведено сообщение с ошибкой
  ...
  _hideInputError(inputListElement) {
    const inputErrorContainer = this._formElement.querySelector(`.${inputListElement.id}-error`); // Находим класс спана для конкретного поля
...
const formProfilValidator = new FormValidator(formValidationConfig, formElementProfile);//создаю экземпляр карда
formProfilValidator.enableValidation();

const formElementCardValidator = new FormValidator(formValidationConfig, formElementCard);//создаю экземпляр карда
